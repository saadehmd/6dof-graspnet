...
def pcl_to_ros(pcl_array , frame_id):
...
def get_color_list(cluster_count):
    ...
    """
    color_list = []
    for i in xrange(0, cluster_count):
    	color_list.append(random_color_gen())
    return color_list



def plane_seg(cloud_arr):
    ''' 
		Returns a pointcloud after removing inliers of
		
		a planar model segmented from the input cloud
    '''
	
    cloud = pcl.PointCloud()
    cloud_arr = np.array(cloud_arr, dtype = np.float32)
    cloud.from_array(cloud_arr)

    n_points = cloud_arr.shape[0]
    ...
    while cloud_arr.shape[0] > 0.3*n_points:
        ...
        cloud_arr = np.delete(cloud_arr, indices, axis = 0)
...
def moving_least_squares(cloud, search_rad):
	...
	mls.set_search_radius(search_rad)
